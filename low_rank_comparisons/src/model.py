...
from mltd.tarp import DyLRLinear
...
class LowRankConv1D(nn.Module):
    def __init__(self, nf, nx, rank=4, tarp='vanilla'):
        ...
        if tarp == 'vanilla':
            self.adapter = LRLayer(nf, nx, rank)
            self.shift_weights = True
        elif tarp == 'kronecker':
            self.adapter = KroneckerLayer(nf, nx, rank)
            self.shift_weights = True
        elif tarp == 'dynamic':
            self.adapter = DyLRLayer(nf, nx, rank)
...
class Attention(nn.Module):
    def __init__(self, nx, n_ctx, config, scale=False):
        ...
        if config.lowrank > 0:
            ...
            self.c_attn = DyLRLinear(n_state * 3, nx, config.lowrank, scale_weight=True)
            self.c_proj = DyLRLinear(n_state, nx, config.lowrank, scale_weight=True)
        else:
...
class MLP(nn.Module):
    def __init__(self, n_state, config):  # in MLP: n_state=3072 (4 * n_embd)
        ...
        if config.lowrank > 0:
            ...
            self.c_fc = DyLRLinear(n_state, nx, config.lowrank, scale_weight=True)
            self.c_proj = DyLRLinear(nx, n_state, config.lowrank, scale_weight=True)
        else:
